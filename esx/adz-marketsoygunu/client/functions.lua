			...
			if IsControlJustPressed(0, 22) then
				if dialRotation == currentSafeCorrectPosition then
					...
					if #safeLocks == 1 then
						...
						TriggerServerEvent("esx_phone:send", "police", "GPS Market Soygunu İhbarı! Konumu Haritada işaretlemek için Buraya Basın", true, { ["x"] = x, ["y"] = y, ["z"] = z })
					end
...
TakeSafeItem = function(itemEntity, safeData)
	...
	if DoesEntityExist(itemEntity) then
		...
		local stoleItem = GetEntityModel(itemEntity) == GetHashKey("prop_anim_cash_pile_02") and "money" or "bread"
