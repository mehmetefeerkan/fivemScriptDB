...
CreateThread(function()
	while true do
		...
		if objects.object ~= nil and objects.ObjectVertX ~= nil and objects.ObjectVertY ~= nil and objects.ObjectVertZ ~= nil and objects.OjbectDir ~= nil and objects.isBed ~= nil then
			...
			if GetDistanceBetweenCoords(objectcoords.x, objectcoords.y, objectcoords.z,getPlayerCoords) < 1.5 and not using then
				if objects.isBed == true then
					DrawText3Ds(objectcoords.x, objectcoords.y, objectcoords.z+0.30, "[~g~E~w~] "..anim.." uzan")
					DrawText3Ds(objectcoords.x, objectcoords.y, objectcoords.z+0.20, "~w~ Yatma biçimini degistirmek için ~g~ok tuslarını~w~ kullan")
				...
				-- else
				-- 	DrawText3Ds(objectcoords.x, objectcoords.y, objectcoords.z+0.30, " [~g~G~w~] Otur")
			...
			if using == true then
				Draw2DText("[~g~F~w~] Kalk",0,1,0.5,0.92,0.6,255,255,255,255)
...
CreateThread(function()
	while true do
		...
		if using then
			...
			TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Stresin azalıyor.', length = 500})
		end
	end
end)
