		...
		if dist < 1.5 then
			if RLConfig.Doors[k].pickable then
				if RLConfig.Doors[k].locked then
					...
					exports['mythic_notify']:SendAlert('error', 'Kapı zaten açık.')
				end
			else
				exports['mythic_notify']:SendAlert('error', 'Bu kapıda aslında kilidini açabileceğinden daha güçlü bir kilidi var.')
			end
		end
	end
end)
...
function lockpickFinish(success)
    ...
    if success then
		exports['mythic_notify']:SendAlert('success', 'Başarılı')
		setDoorLocking(closestDoorValue, closestDoorKey)
    elseif chance >= 15 then
		exports['mythic_notify']:SendAlert('error', 'Maymuncuk kırıldı.')
