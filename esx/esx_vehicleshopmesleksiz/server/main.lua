        ...
        function(result)
            if result[1] ~= nil then
				...
				if myID == CarOwner then
					...
					data = {}
						TriggerClientEvent('chatMessage', other, "^4VPlakalı Araç ^*^1" .. plate .. "^r^4tarafindan sana teslim edildi: ^*^2" .. myName)
			 
						MySQL.Sync.execute("UPDATE owned_vehicles SET owner=@owner WHERE plate=@plate", {['@owner'] = targetSteam, ['@plate'] = plate})
						TriggerClientEvent('chatMessage', source, "^4Aracinizi ^*^3plaka^0^4 ile teslim ettiniz ^*^1" .. plate .. "\" ^r^4teslim edilen ^*^2".. targetSteamName)
				else
					print("Transfer Etmedi")
					TriggerClientEvent('chatMessage', source, "^*^1Araca sahip degilsiniz")
				end
			else
				TriggerClientEvent('chatMessage', source, "^1^*ERROR: ^r^0Bu arac plakasi mevcut degil veya yanlisa yazilmis.")
            end
