...
function DrawText3D(coords, text, size)
	...
	local onScreen,_x,_y=World3dToScreen2d(coords.x,coords.y,coords.z + 0.0)
...
function JailLogin()
	...
	ESX.ShowNotification("En son uyuduğunda hapise atılmışsın, beklemeye devam et!")
...
function UnJail()
	...
	ESX.ShowNotification("Artık özgürsün, uslu dur!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Hapisten çıkmana " .. jailTime .. " Ay kaldı!")
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Yemek Hazırla"

					if not v["state"] then
						PackageText = "Bugünlük bu kadar yeter"
					end

					DrawText3D(v, "[E] " .. PackageText, 0.4)
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Bu paketi zaten aldın!")
							end
...
function PackPackage(packageId)
	...
	local PackageObject = CreateObject(GetHashKey("prop_food_bs_tray_02"), Package["x"], Package["y"], Package["z"], true)
	...
	while Packaging do
		...
		local TimeToTake = 38500 * 1.20 -- Minutes
		...
		if not IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_BUM_BIN") then
			...
			ESX.ShowNotification("İptal edildi!")
		...
		if PackPercent >= 100 then
			...
			DrawText3D(Package, "Menü hazırlanıyor... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			DrawText3D(DeliverPosition, "[E] Menüyü Teslim et", 0.4)
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					TriggerEvent('malfonzillo:jail:sureazalt')
				end
			end
		end
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'top-left',
			elements = {
				{ label = "Yakındaki Kişiyi Hapise Gönder", value = "jail_closest_player" },
				{ label = "Hapisten Kişi Çıkar", value = "unjail_player" }
			}
		}, 
	function(data, menu)
            	...
            	if jailTime == nil then
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Bir şeyler koymalısın!")
						  	else
								menu3.close()
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									  --ESX.ShowNotification("No players nearby!")
									  exports['mythic_notify']:SendAlert('inform', 'Yakında oyuncu yok!', 2500)
								else
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Hapiste kimse yok!!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Mahkum: " .. playerArray[i].name .. " | Hapis Süresi: " .. playerArray[i].jailTime .. " dakika", value = playerArray[i].identifier })
				end
...
RegisterNetEvent('malfonzillo:jail:sureazalt')
AddEventHandler('malfonzillo:jail:sureazalt', function()
        jailTime = jailTime - 1
        ESX.ShowNotification("Hapisten çıkmana " .. jailTime .. " Ay kaldı!")
        TriggerServerEvent("esx-qalle-jail:updateJailTime", jailTime)
        if jailTime == 0 then
