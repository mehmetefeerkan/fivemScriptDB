...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if not pumpObject then
			if GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 1 >= 0 then
				currentFuel = oldFuel + fuelToAdd

				--SetPedAmmo(ped, 883325847, math.floor(GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 1))
			else
				isFueling = false
			end
		else
