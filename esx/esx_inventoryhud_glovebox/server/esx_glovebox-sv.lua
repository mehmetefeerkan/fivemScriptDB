            ...
            for i = 1, #coffres, 1 do
              if coffres[i].name == item then
                if (coffres[i].count >= count and count > 0) then
				  ...
				  MySQL.Async.execute('INSERT INTO bz_bagajlog (Hangisi, Sahip, Type, Miktar, Eylem)VALUES (@Hangisi, @Sahip, @Type, @Miktar, @Eylem) ', {
					['@Hangisi']   = 'TORPIDO',
					['@Sahip']   = xPlayer.name,
					['@Type']   = item,
					['@Miktar']    = count,
					['@Eylem']    = 'Torpidodan Aldı'})
      ...
      if (playerAccountMoney >= count and count > 0) then
        TriggerEvent(
          ...
          function(store)
			...
			TriggerClientEvent('notification', _source, 'Torpidoya para koyamazsın!', 2)
              -- Checks passed. Storing the item.
