...
AddEventHandler('esx_property:rentProperty', function(propertyName)
	...
	local rent     = ESX.Math.Round(property.price)
		...
		TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if xPlayer.canCarryItem(item, count) then
					...
					MySQL.Async.execute('INSERT INTO log_ev (Sender, Type, Amount, Reciever)VALUES (@Sender, @Type, @Amount, @Reciever) ', {
					['@Sender']   = xPlayer.name,
					['@Type']   = item,
					['@Amount']    = count,
					['@Reciever']    = 'Motelden Aldi'})
