...
function CreateBlips()
	...
	for i=1, #ownedProperties, 1 do
		local property = ownedProperties[i]
		...
		if property.entering then
			...
			SetBlipScale  (Blips[property.name], 0.1)
...
function SetPropertyOwned(name, owned)
	...
	if owned then
		...
		SetBlipScale  (Blips[enteringName], 0.1)
...
function OpenRoomMenu(property, owner)
	...
	if CurrentPropertyOwner == owner then
		...
		table.insert(elements, {label = 'Dolabini ac',  value = 'room_inventory'})
