...
function OpenCloakroomMenu()
	...
	elseif grade == 'lieutenant1' then
		table.insert(elements, {label = _U('police_wear'), value = 'lieutenant1_wear'})
	elseif grade == 'lieutenant2' then
		table.insert(elements, {label = _U('police_wear'), value = 'lieutenant2_wear'})
	elseif grade == 'captain' then
		table.insert(elements, {label = _U('police_wear'), value = 'captain_wear'})
	elseif grade == 'captain2' then
		table.insert(elements, {label = _U('police_wear'), value = 'captain2_wear'})
	elseif grade == 'commander' then
		table.insert(elements, {label = _U('police_wear'), value = 'commander_wear'})
	elseif grade == 'asistantchief' then
		table.insert(elements, {label = _U('police_wear'), value = 'asistantchief_wear'})
	elseif grade == 'chief' then
		table.insert(elements, {label = _U('police_wear'), value = 'chief_wear'})
	elseif grade == 'hsu' then
		table.insert(elements, {label = _U('police_wear'), value = 'hsu_wear'})

	elseif grade == 'srecruit' then
		table.insert(elements, {label = _U('police_wear'), value = 'srecruit_wear'})
	elseif grade == 'sofficer' then
		table.insert(elements, {label = _U('police_wear'), value = 'sofficer_wear'})
	elseif grade == 'ssergeant' then
		table.insert(elements, {label = _U('police_wear'), value = 'ssergeant_wear'})
	elseif grade == 'sintendent' then
		table.insert(elements, {label = _U('police_wear'), value = 'sintendent_wear'})
	elseif grade == 'slieutenant' then
		table.insert(elements, {label = _U('police_wear'), value = 'slieutenant_wear'})
	elseif grade == 'schef' then
		table.insert(elements, {label = _U('police_wear'), value = 'schef_wear'})
	elseif grade == 'sboss' then
		table.insert(elements, {label = _U('police_wear'), value = 'sboss_wear'})
	end

	if grade == 'recruit' or grade == 'officer' or grade == 'sergeant' or grade == 'intendent' or grade == 'lieutenant'or grade == 'chef' or grade == 'boss' or grade == 'lieutenant1' or grade == 'lieutenant2' or grade == 'captain' or grade == 'captain2' or grade == 'commander' or grade == 'asistantchief' or grade == 'chief' then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if
			...
			data.current.value == 'asistantchief_wear' or
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									Citizen.Wait(5000)
									ClearPedTasks(playerPed)
										TriggerServerEvent('esx_policejob:heal', GetPlayerServerId(closestPlayer), 'small')
					...
					elseif action == 'uncuff' then
						--TriggerServerEvent('esx_policejob:uncuff', GetPlayerServerId(closestPlayer))
...
AddEventHandler('esx_policejob:getuncuffed', function(playerheading, playercoords, playerlocation)
	...
	Citizen.Wait(5500)
	IsHandcuffed = false
	TriggerEvent('esx_policejob:uncuff')
	ClearPedTasks(GetPlayerPed(-1))
end)
--------------------DEXA------------------
...
Citizen.CreateThread(function()
	while true do
			if PlayerData.job == 'police' and PlayerData.getInventoryItem('policegps').count >= 1 then
				TriggerEvent('esx_policejob:updateBlip')
			else
				TriggerEvent('esx_policejob:updateBlip')
			end
...
AddEventHandler('esx_policejob:updateBlip', function()
	...
	if not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'ambulance' and PlayerData.job.name == 'police' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
...
RegisterNetEvent('brn:ybindir')
  AddEventHandler('brn:ybindir', function()
      local playerPed = PlayerPedId()
      local coords = GetEntityCoords(playerPed)
...
RegisterCommand("ybindir",function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 2.0 then
        ...
        if isPedDead then
            TriggerServerEvent('brn:yaralibindir', GetPlayerServerId(closestPlayer))
        else
            exports['mythic_notify']:SendAlert('inform', 'Kişi yaralı değil.', 6500)
        end
        end, GetPlayerServerId(closestPlayer))   
    else
        exports['mythic_notify']:SendAlert('inform', 'Yakında oyuncu yok.', 6500)
    end   
end)



RegisterNetEvent('brn:yaralıindir')
AddEventHandler('brn:yaralıindir', function()
    local playerPed = PlayerPedId()
    if not IsPedSittingInAnyVehicle(playerPed) then
        return
    end
...
RegisterCommand("yindir",function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 2.0 then
        ESX.TriggerServerCallback('policejob:isplayerdead', function(isPedDead)
            if isPedDead then
                TriggerServerEvent('brn:yaralindir', GetPlayerServerId(closestPlayer))
            else
                exports['mythic_notify']:SendAlert('inform', 'Kişi yaralı değil.', 6500)
            end
        end, GetPlayerServerId(closestPlayer))   
