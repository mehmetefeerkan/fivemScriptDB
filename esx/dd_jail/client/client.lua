...
AddEventHandler("esx:playerLoaded", function(newData)
	...
	ESX.TriggerServerCallback("dd_jail:retrieveJailTime", function(inJail, newJailTime)
		if inJail then
...
RegisterNetEvent("dd_jail:openJailMenu")
AddEventHandler("dd_jail:openJailMenu", function()
	OpenJailMenu()
end)

RegisterNetEvent("dd_jail:jailPlayer")
AddEventHandler("dd_jail:jailPlayer", function(newJailTime)
	jailTime = newJailTime

	Cutscene()
end)

RegisterNetEvent("dd_jail:unJailPlayer")
AddEventHandler("dd_jail:unJailPlayer", function()
	jailTime = 0
...
function JailLogin()
	...
	exports['okokNotify']:Alert("", "Last time you went to sleep you were jailed, because of that you are now put back!", 5000, 'success')
	InJail()
end
...
function UnJail()
	...
	exports['okokNotify']:Alert("", "You are released, stay calm outside! Good LucK!", 5000, 'success')
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			exports['okokNotify']:Alert("", "You have " .. jailTime .. " minutes left in jail!", 5000, 'success')

			TriggerServerEvent("dd_jail:updateJailTime", jailTime)

			if jailTime == 0 then
				UnJail()

				TriggerServerEvent("dd_jail:updateJailTime", 0)
			end
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								exports['okokNotify']:Alert("", "You've already taken this package!", 5000, 'success')
							end
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if not IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_BUM_BIN") then
			...
			exports['okokNotify']:Alert("", "Canceled!", 5000, 'success')
		...
		if PackPercent >= 100 then
			...
			local finished = exports["taskbar"]:taskBar(TimeToTake,"Packaging")
		end
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					TriggerServerEvent("dd_jail:prisonWorkReward")
				end
			end
		end
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
					exports['okokNotify']:Alert("", "The time needs to be in minutes!", 5000, 'success')
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						exports['okokNotify']:Alert("", "No players nearby!", 5000, 'success')
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								exports['okokNotify']:Alert("", "Message", 5000, 'success')
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("dd_jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					exports['okokNotify']:Alert("", "Your jail is empty!", 5000, 'success')
					return
				end
