...
function Cutscene()
	...
	TriggerEvent('skinchanger:getSkin', function(skin)
		if GetHashKey(GetEntityModel(PlayerPedId())) == Male then
			...
			TriggerEvent('skinchanger:loadClothes2', skin, clothesSkin)
	...
	TriggerServerEvent("dd_sound_sv:PlayOnSource", "cell", 0.3)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Boiling Broke" or action == "Security" then
				...
				if PlayerData.job.name ~= "police" then
					exports['okokNotify']:Alert("", "You don't have an key to go here!", 5000, 'success')
					return
				end
			end
