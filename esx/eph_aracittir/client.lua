...
function DrawText3D(coords, text, size)
	...
	local scale = (size / dist) * 0.1
	local fov   = (0.1 / GetGameplayCamFov()) * 100
    ...
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) then
                   DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Aracı ittirmek için [~r~SHIFT~w~] ve [~r~E~w~] tuşlarına basılı tutun.', 0.6) 
                end
