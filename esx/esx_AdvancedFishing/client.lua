...
local blip = AddBlipForCoord(Config.SellFish.x, Config.SellFish.y, Config.SellFish.z)
			...
			AddTextComponentString("Balık Satışı")
...
local blip2 = AddBlipForCoord(Config.SellTurtle.x, Config.SellTurtle.y, Config.SellTurtle.z)
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Deniz Kaplumbağası Satışı")
...
local blip3 = AddBlipForCoord(Config.SellShark.x, Config.SellShark.y, Config.SellShark.z)
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Köpekbalığı Satışı")
...
for _, info in pairs(Config.MarkerZones) do
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("Bot kiralama")
	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Keys['X']) then
				...
				ESX.ShowNotification("Balık tutmayı bıraktın.")
			end
			if fishing then
	...
	while true do
		...
		Wait(wait)
			if fishing then
				...
				ESX.ShowNotification("Balık yemi yutuyor. \n " .. correct .. " basarak yakalamayı deneyebilirsin.")
				input = 0
				pausetimer = 0
			end
...
AddEventHandler('fishing:fishstart', function()
	...
	if IsPedInAnyVehicle(playerPed) then
		ESX.ShowNotification("Araç içerisindeyken balık tutamazsın.")
	else
		if pos.y >= 7700 or pos.y <= -4000 or pos.x <= -3700 or pos.x >= 4300 then
			ESX.ShowNotification("Balık tutmaya başladın.")
			TaskStartScenarioInPlace(playerPed, "WORLD_HUMAN_STAND_FISHING", 0, true)
			fishing = true
		else
			ESX.ShowNotification("Sahilden biraz uzaklaşman gerekiyor.")
		end
	end
    ...
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
					...
					DisplayHelpText('E tuşuna basarak bot kiralayabilirsin.')
...
function OpenBoatsMenu(x, y , z)
	...
	if data.current.value == 'boat' then
		...
		TriggerEvent("chatMessage", 'Bot kiraladın:', {255,0,255}, '$' .. 2500)
	...
	if data.current.value == 'boat2' then
		...
		TriggerEvent("chatMessage", 'Bot kiraladın:', {255,0,255}, '$' .. 5500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat3' then
		...
		TriggerEvent("chatMessage", 'Bot kiraladın:', {255,0,255}, '$' .. 6000)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat4' then
		...
		TriggerEvent("chatMessage", 'Bot kiraladın:', {255,0,255}, '$' .. 7500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat5' then
		...
		TriggerEvent("chatMessage", 'Bot kiraladın:', {255,0,255}, '$' .. 4500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat6' then
		...
		TriggerEvent("chatMessage", 'Bot kiraladın:', {255,0,255}, '$' .. 3500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'police' then
		...
		TriggerEvent("chatMessage", 'Botu indirdin.')
		SetPedCoordsKeepVehicle(ped, x, y , z)
