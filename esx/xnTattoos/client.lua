...
Citizen.CreateThread(function()
	AddTextEntry("ParaTattoos", "Dövme Dükkanı")
...
function RemoveTattoo(name, label)
	...
	ESX.ShowNotification( GetLabelText(label) .. " adlı dövmeni kaldırdın")
end

function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "Dövme Dükkanını Aç", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Kamera görüntüsünü değiştir", 21, "Opaklığı değiştir", {90, 89}, "Dövme Satın Al/Sildir", 191)
		scaleString = sType
	end
end

Citizen.CreateThread(function()
	JayMenu.CreateMenu("tattoo", "Dövme Dükkanı", function()
        return CloseTattooShop()
    end)
    JayMenu.SetSubTitle('tattoo', "Kategoriler")
