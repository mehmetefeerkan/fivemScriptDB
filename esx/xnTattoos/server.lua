...
ESX.RegisterServerCallback('SmallTattoos:PurchaseTattoo', function(source, cb, tattooList, price, tattoo, tattooName)
	...
	if xPlayer.getMoney() >= price then
		...
		TriggerClientEvent('esx:showNotification', source, "" .. tattooName .. " adlı dövmeyi" .. price .. "$ karşılığında aldın")
		cb(true)
	else
		TriggerClientEvent('esx:showNotification', source, "Bu dövme için yeteri kadar paran yok")
		cb(false)
	end
end)
