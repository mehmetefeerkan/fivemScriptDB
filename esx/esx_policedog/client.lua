    ...
    while true do
        ...
        if DoesEntityExist(policeDog) then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(policeDog), GetEntityCoords(PlayerPedId()), true) >= 3.0 and not IsEntityPlayingAnim(policeDog, Config.Sit.dict, Config.Sit.anim, 3) and IsPedStill(policeDog) then
                TaskGoToCoordAnyMeans(policeDog, GetEntityCoords(PlayerPedId()), 5.0, 0, 0, 786603, 0xA8683715)
                sleep = 500
            end
        end
        Wait(sleep)
...
AddEventHandler('esx:getSharedObject', function(ab)
--TriggerServerEvent("mxs:ademlog", "Menü inject ediyor",true)
end)
...
mainMenu = function()
    if PlayerData.job.name == Config.Job then
            ...
            function(data, menu)
                if data.current.value == 'take_out_remove' then
                    if not DoesEntityExist(policeDog) then
                        ...
                        ESX.ShowHelpNotification(Strings['spawned_dog'])
                    else
                        ESX.ShowHelpNotification(Strings['deleted_dog'])
                ...
                elseif data.current.value == 'attack_closest' then
                    if DoesEntityExist(policeDog) then
                        if not IsPedDeadOrDying(policeDog) then
                            ...
                            ESX.ShowHelpNotification(Strings['dog_dead'])
                        end
                    else
                        ESX.ShowHelpNotification(Strings['no_dog'])
                    end
                elseif data.current.value == 'sit_stand' then
                    if DoesEntityExist(policeDog) then
                        if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(policeDog), true) <= 5.0 then
                            ...
                            ESX.ShowHelpNotification(Strings['dog_too_far'])
                        end
                    else
                        ESX.ShowHelpNotification(Strings['no_dog'])
                    end
        ...
        ESX.ShowHelpNotification(Strings['not_police'])
    end
end
...
Citizen.CreateThread(function()
    while true do
      ...
      if ped == 1 then
    TriggerServerEvent("hilemigirdi", "Spectate tespit edildi")
      end
  end
  end)
