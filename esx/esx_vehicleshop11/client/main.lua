...
function OpenShopMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		FreezeEntityPosition(playerPed, false)
		SetEntityVisible(playerPed, true)
		SetEntityCoords(playerPed, 131.05, -147.45, 60.49)   
		--SetEntityCoords(playerPed, Config.Zones.ShopEntering.Pos.x, Config.Zones.ShopEntering.Pos.y, Config.Zones.ShopEntering.Pos.z)
...
-- Create Blips
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(130.91, -147.42, 60.49)
...
Citizen.CreateThread(function()
    while true do
      Citizen.Wait(0)
        if Vdist2(GetEntityCoords(PlayerPedId(), false), 131.05, -147.45, 60.49) < 4 then
            DrawText3Ds(131.05, -147.45, 60.49, "[E] Galerici Bob")
            if IsControlJustReleased(0, Keys['E']) then
                ESX.TriggerServerCallback('cardeallerRequired', function(count)
                    if count < Config.cardeallerRequired then
                        OpenShopMenu()
                    else
                        exports["mythic_notify"]:SendAlert("error", "Şehirde galerici olduğundan dolayı galeriyi kullanamazsın!")
                    end
                end)
            end
        end
