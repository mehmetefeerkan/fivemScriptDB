...
local bankLocations = {
    {i = 108, c = 0, x = 237.1703, y = 217.8410, z = 106.28, s = 0.8, n = "Pacific Bank"}, -- blip id, blip color, x, y, z, scale, name/label
...
Citizen.CreateThread(function()
    while true do
        ...
        if playerNearBank() and not inMenu then
            DisplayHelpText("Bankamıza ~INPUT_PICKUP~ Tuşunu Kullanarak Ulaşabiliriniz.")
...
RegisterCommand('atm', function(source, args) -- Command to access ATM when players are near instead of spam notifications when near an ATM
    if playerNearATM() then
        ...
        exports['mythic_notify']:DoHudText('error', 'ATM ye yakın değilsin.')
    end
end)
...
function openPlayersBank(type, color)
    ...
    exports['progressBars']:startUI(time, "İşlem Yapılıyor...")
    Citizen.Wait(time)
    ClearPedTasks(ped)
    if type == 'bank' then
        inMenu = true
...
RegisterNUICallback('deposit', function(data)
    if not atATM then
        ...
        exports['mythic_notify']:DoHudText('error', 'ATM ye Para Yatıramazsın En Yakın Bankamıza Gelin.')
    end
end)
...
function closePlayersBank()
    ...
    TaskPlayAnim(ped, dict, anim, 8.0, 8.0, -1, 0, 0, 0, 0, 0)
    exports["hsn-bar"]:taskBar(3000, "Görüşmek Üzere...")
    Citizen.Wait(time)
    ClearPedTasks(ped)
    inMenu = false
end
