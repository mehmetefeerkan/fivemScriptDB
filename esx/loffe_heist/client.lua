...
Citizen.CreateThread(function()
    function Initialize(scaleform)
        ...
        PushScaleformMovieFunctionParameterString("Bilgisayarım")
        PopScaleformMovieFunctionVoid()
        ...
        PushScaleformMovieFunctionParameterFloat(6.0)
        PushScaleformMovieFunctionParameterString("Bilgisayarı Kapat")
        PopScaleformMovieFunctionVoid()

        PushScaleformMovieFunction(scaleform, "SET_MidnightS")
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                elseif Hacking and ProgramID == 86 then
                    ...
                    mHacking = false
                    --TriggerServerEvent('esx_phone:send', 'police', '' .. v.BankName .. ' bankasında soygun ihbarı!!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
					if ESX.GetPlayerData().job.name == "police" then
						TriggerEvent("chat:addMessage", {
							template = '<div class="chat-message server"> [İHBAR]: {0}</div>',
							args = {"Merkez bankasında soygun ihbarı! Tüm ekipler intikal etsin."}
						})
                ...
                if Hacking then
                    PushScaleformMovieFunction(scaleform, "SHOW_MidnightS")
                    PushScaleformMovieFunctionParameterBool(true)
                    PopScaleformMovieFunctionVoid()
                    if lives <= 0 then
                        SorF = true
                        ...
                        Wait(5000)
                        local v = Config.BankRobbery[hackingBank]
                        --TriggerServerEvent('esx_phone:send', 'police', 'İsimsiz bir şahıs ' .. v.BankName .. 'bankasının sistemlerini hacklemeye çalıştı!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
						if ESX.GetPlayerData().job.name == "police" then
							TriggerEvent("chat:addMessage", {
								template = '<div class="chat-message server"> [İHBAR]: {0}</div>',
								args = {"İsimsiz bir şahıs Merkez bankasının sistemini hacklemeye çalıştı!"}
							})
							PlaySound(-1, "Event_Message_Purple", "GTAO_FM_Events_Soundset", 0, 0, 1)
							PlaySound(-1, "Boss_Message_Orange", "GTAO_Boss_Goons_FM_Soundset", 0, 0, 1)
						end
...
function USBSuccess(success, timeremaining)
    if success then
       ...
       -- TriggerServerEvent('esx_phone:send', 'police', 'İsimsiz bir şahıs ' .. v.BankName .. 'bankasının sistemlerine sızdı!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
		if ESX.GetPlayerData().job.name == "police" then
			TriggerEvent("chat:addMessage", {
				template = '<div class="chat-message server"> [İHBAR]: {0}</div>',
				args = {"İsimsiz bir şahıs Merkez bankasının sistemine sızdı!!!"}
			})
			PlaySound(-1, "Event_Message_Purple", "GTAO_FM_Events_Soundset", 0, 0, 1)
			PlaySound(-1, "Boss_Message_Orange", "GTAO_Boss_Goons_FM_Soundset", 0, 0, 1)
		end
    ...
    while true do
        for k, v in pairs(Config.BankRobbery) do
            ...
            if GetDistanceBetweenCoords(coords, v.Hacking.Coords, true) <= 2.0 and v.Doors[#v.Doors].Frozen and not mHacking then
                ...
                if allowed then
                    TriggerEvent('loffe_heist:helpTimed', 'Sisteme sızmak için ~INPUT_CONTEXT~ tuşuna bas!', 25)
                    if IsControlPressed(0, 38) then
                        ...
                        if police >= v.Cops then
                            ...
                            ESX.ShowNotification('Polis sayısı yetersiz!')
                        end
                    end
                end
                Wait(10)
        ...
        for k, v in pairs(Config.BankRobbery) do
            ...
            if GetDistanceBetweenCoords(coords, v.Money.BoxPosition, true) <= 1.5 and not v.Doors[#v.Doors].Frozen then
                if v.Money.Amount > 0 and bag then
                    ...
                    AddTextComponentSubstringPlayerName('Paraları toplamak için ~INPUT_CONTEXT~ tuşuna basın.')
                    ...
                    if IsControlPressed(0, 38) then
                        ...
                        if police >= v.Cops then
                            ...
                            while v.Money.Amount > 0 do
                                ...
                                TriggerEvent('loffe_heist:helpTimed', 'Para toplamayı durdurmak için ~INPUT_JUMP~ tuşuna basılı tutun', 100)
                                Wait(350)
                ...
                if GetDistanceBetweenCoords(coords, d.Coords, true) <= 2.5 and d.Frozen then
                    if hasWeld then
                        while GetDistanceBetweenCoords(GetEntityCoords(player), d.Coords, true) <= 2.5 and d.Frozen and not d.Hacking do
                            ...
                            Wait(50)
                            if allowed then
                                TriggerEvent('loffe_heist:helpTimed', 'Kapıyı eritmek için ~INPUT_CONTEXT~ tuşuna basınız.', 50)
                                if IsControlPressed(0, 38) then
                                    local police = nil
                                    ESX.TriggerServerCallback('loffe_heist:getCops', function(cops)
                                        police = cops
                                    ...
                                    if police >= v.Cops then
                                        ...
                                        local welded = true
                                        for i = 1, 100 do
                                            TriggerEvent('loffe_heist:text', '~r~[~b~SPACE~r~] ~g~Iptal', 0.955, 0.97, d.Time)
                                            drawSub('Eritme ~b~' .. i .. '%', 4,1,0.5,0.96,0.6,255,255,255,255, d.Time)
                                            if IsDisabledControlPressed(0, 22) then
                                                ClearPedTasks(player)
                                                blockKeys = false
                                                welded = false
                ...
                if not s.Looted then
                    if hasDrill then
                        if GetDistanceBetweenCoords(coords, s.Coords, true) <= 1.0 and not Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Frozen then
                            TriggerEvent('loffe_heist:helpTimed', 'Kasayı açmak için ~INPUT_CONTEXT~ tuşuna basın.', 5)
                            if IsControlPressed(0, 38) then
                                local police = nil
                                ESX.TriggerServerCallback('loffe_heist:getCops', function(cops)
                                    police = cops
...
Citizen.CreateThread(function()
    while true do
        while drilling do
            local text = '~INPUT_CELLPHONE_UP~ İleri \n~INPUT_CELLPHONE_DOWN~ Geri\n~INPUT_JUMP~ İptal Et'
				...
				if not Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Frozen then
                    if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Coords) <= 5 then
                        while GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Coords) <= 5 do
                            TriggerEvent('loffe_heist:helpTimed', 'Kapıyı kapatmak için ~INPUT_CONTEXT~ tuşuna bas.', 25)
