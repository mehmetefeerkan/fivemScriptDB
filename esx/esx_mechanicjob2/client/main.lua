            ...
            ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
              ...
              ESX.UI.Menu.Open(
                ...
                function(data, menu)
                  ...
                  ESX.Game.SpawnVehicle(vehicleProps.model, Config.Jobs[PlayerData.job.name].VehicleSpawnPoint.Pos, 270.0, function(vehicle)
                    ESX.Game.SetVehicleProperties(vehicle, vehicleProps)
                    local playerPed = GetPlayerPed(-1)
                    TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
                  end)

                  TriggerServerEvent('esx_society:removeVehicleFromGarage', PlayerData.job.name, vehicleProps)
              ...
              function(data, menu)
                for i=1, #elements, 1 do
                  if Config.MaxInService == -1 then
                    ESX.Game.SpawnVehicle(data.current.value, Config.Jobs[PlayerData.job.name].VehicleSpawnPoint.Pos, 90.0, function(vehicle)
                      local playerPed = GetPlayerPed(-1)
                      TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
                    end)
                    break
...
function OpenMobilemechanicActionsMenu()
  ...
  ESX.UI.Menu.Open(
	...
	function(data, menu)
      ...
      if data.current.value == 'billing' then
        ...
        menu.close()
                TriggerServerEvent('esx_billing:sendBillbezdumsur', GetPlayerServerId(closestPlayer), 'society_'..PlayerData.job.name, Config.Jobs[PlayerData.job.name].telefon, amount)
              end
            end
          end,
        function(data, menu)
      ...
      --[[if data.current.value == 'hijack_vehicle' then
        ...
        exports['progressBars']:startUI(10000, "Araba Dızlanıyor...")
  ...
  if data.current.value == 'fix_vehicle' then
      ...
      if DoesEntityExist(vehicle) then
        ESX.TriggerServerCallback("esx_mechanicjob:checkItem", function(result)
          if result == true then
            IsBusy = true
            TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_BUM_BIN", 0, true)
            exports['progressBars']:startUI(20000, "Tamir Ediliyor...")
            Citizen.CreateThread(function()
              Citizen.Wait(20000)
          ...
          ESX.ShowNotification("Tamir kitin olmalı")
        end
        end)
      else
        ESX.ShowNotification(_U('no_vehicle_nearby'))
	...
	elseif data.current.value == 'clean_vehicle' then
		...
		if DoesEntityExist(vehicle) then
      IsBusy = true
      exports['progressBars']:startUI(10000, "Temizleniyor...")
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              menu2.close()
              menu.close()
              TriggerServerEvent('esx_mechanicjob:putStockItems', itemName, count, PlayerData.job.name)
...
Citizen.CreateThread(function()
  for k, v in pairs(Config.Jobs) do
    ...
    AddTextComponentString(v.telefon)
...
Citizen.CreateThread(function()
  while true do
    Wait(0)
    if PlayerData.job ~= nil and (PlayerData.job.name == 'mekanik4' or PlayerData.job.name == 'mechanic'or PlayerData.job.name == 'mechanic3'or PlayerData.job.name == 'mekanik2') then

      local coords = GetEntityCoords(GetPlayerPed(-1))

      for k,v in pairs(Config.Jobs[PlayerData.job.name].locations) do
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentAction ~= nil then
          ...
          if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and IsEligable() then
            ...
            if CurrentAction == 'delete_vehicle' then
              ...
              if Config.EnableSocietyOwnedVehicles then
                ...
                TriggerServerEvent('esx_society:putVehicleInGarage', PlayerData.job.name, vehicleProps)
        ...
        if IsControlJustReleased(0, Keys['F6']) and not IsDead and PlayerData.job ~= nil and IsEligable() then
