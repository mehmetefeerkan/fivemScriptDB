...
for k, v in pairs(Config.Jobs) do
  TriggerEvent('esx_phone:registerNumber', k, v.telefon, true, true)
...
ESX.RegisterServerCallback("esx_mechanicjob:checkItem", function(source, cb)
  local xPlayer = ESX.GetPlayerFromId(source)
  local item = xPlayer.getInventoryItem("tamir_kiti")

  if item.count >= 1 then
    xPlayer.removeInventoryItem("tamir_kiti", 1)
    cb(true)
  else
    cb(false)
  end
...
ESX.RegisterServerCallback('esx_mechanicjob:getStockItems', function(source, cb, job)
...
AddEventHandler('esx_mechanicjob:putStockItems', function(itemName, count, job)
