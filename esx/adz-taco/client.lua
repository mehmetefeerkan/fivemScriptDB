...
Citizen.CreateThread(function()
    while true do
        ...
        if distance4 < 10 then
            ...
            if distance4 < 1 then
                DrawText3D(Config.TacoSatisyeri.x, Config.TacoSatisyeri.y, Config.TacoSatisyeri.z + 0.4, '~g~E~s~ - Siparis Gotur')
                if IsControlJustReleased(0, 38) then
                    TacoSat()
                end
            end
...
function TacoHazirla()
    if not hazirliyormu then
        ESX.TriggerServerCallback('utx-taco:malzemekontrol', function(data)
            hazirliyormu = true
            exports['mythic_progbar']:Progress({
                name = "tacohazirla",
                duration = 10000,
                label = 'Taco hazırlıyorsun...',
                useWhileDead = false,
                canCancel = false,
                controlDisables = {
                    disableMovement = true,
                ...
                if not cancelled then
                    TriggerServerEvent('utx-taco:tacover')
                    hazirliyormu = false
                    ESX.ShowNotification('Başarıyla taco hazırladın!')
                else
                    -- Do Something If Action Was Cancelled
                end
            end)
    ...
    if not paketliyormu then
        ESX.TriggerServerCallback('utx-taco:malzemekontrol', function(data)
            ...
            exports['mythic_progbar']:Progress({
                ...
                if not cancelled then
                    TriggerServerEvent('utx-taco:paketver')
                    paketliyormu = false
                    ESX.ShowNotification('Başarıyla taco paketledin!')
                else
                    -- Do Something If Action Was Cancelled
                end
            end)
...
function TacoSat()
    ...
    elseif satiyormu then
        ESX.ShowNotification('Zaten bir taco satıyorsun!')
    end
end
...
function TacoyuSat()
    exports['mythic_progbar']:Progress({
        ...
        if not cancelled then
            TriggerServerEvent('utx-taco:tacosat')
            satiskordinat = false
            satiyormu = false
            ESX.ShowNotification('Başarıyla taco sattın!')
        else
            -- Do Something If Action Was Cancelled
        end
    end)
