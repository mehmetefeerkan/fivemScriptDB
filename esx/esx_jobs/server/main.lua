		...
		if PlayersWorking[source] == true then
			...
			for i=1, #item, 1 do
				...
				if item[i].name ~= _U('delivery') and itemQtty >= item[i].max then
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = _U('max_limit', item[i].name), length = 5500, style = { ['background-color'] = '#ffffff', ['color'] = '#000000' } })
				elseif item[i].requires ~= "nothing" and requiredItemQtty <= 0 then
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = _U('not_enough', item[1].requires_name), length = 5500})
				else
					if item[i].name ~= _U('delivery') then
