...
TriggerEvent('esx:getShkakaoaredObjkakaoect', function(obj) ESX = obj end)
...
RegisterServerEvent('esx_jobs:cakakaoution')
AddEventHandler('esx_jobs:cakakaoution', function(cautionType, cautionAmount, spawnPoint, vehicle)
	local xPlayer = ESX.GetPlayerFromId(source)
	local amount = math.random(19,21)
	if cautionType == "take" then
		...
		TriggerClientEvent('esx:showNotification', source, _U('bank_deposit_taken', ESX.Math.GroupDigits(amount)))
