...
function OpenActionMenuInteraction(target)
	...
	table.insert(elements, {label = ('Kucağına Al / Bırak'), value = 'drag'})
	ESX.UI.Menu.CloseAll()
	table.insert(elements, {label = ('Sırtına Al / Bırak'), value = 'sirtla'})
	ESX.UI.Menu.CloseAll()
	table.insert(elements, {label = ('Omzuna Al / Bırak'), value = 'omuzla'})
	ESX.UI.Menu.CloseAll()
	table.insert(elements, {label = ('Menüyü Kapat'), value = 'moxha'})
	ESX.UI.Menu.CloseAll()
	ESX.UI.Menu.Open(
    ...
    function(data, menu)
		...
		ESX.UI.Menu.CloseAll()	
		if data.current.value == 'sirtla' then			
			TriggerEvent("moxha:sirtla")
		end
		if data.current.value == 'omuzla' then			
			TriggerEvent("moxha:omuzla")
		end
		if data.current.value == 'drag' then			
			--TriggerServerEvent('esx_barbie_lyftupp:checkRope')
			--exports.pNotify:SendNotification({text = "Birisini Kucağına Aldın", type = "error", timeout = math.random(1000, 5000)})
			--TriggerEvent('esx_barbie_lyftupp:lyfteruppn')
			Citizen.Wait(0)
			if hasRope == true then
				...
				if distance ~= -1 and distance <= 3.0 then
					...
					exports.pNotify:SendNotification({text = "Yakında Kucaklayabilceğin Birisi YOK!", type = "error", timeout = math.random(1000, 5000)})
				end
			else
				--ESX.ShowNotification("İpin yok...")
      end
			menu.close()
		end
...
RegisterNetEvent('esx_barbie_lyftupp:lyfteruppn')
AddEventHandler('esx_barbie_lyftupp:lyfteruppn', function(source)
	exports['mythic_notify']:DoHudText('error', source, ('Birisi seni kucaklamaya çalışıyor istemiyorsan uzaklaş ordan!'))
end)
