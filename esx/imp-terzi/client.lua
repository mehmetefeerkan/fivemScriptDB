...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(2000)
    TriggerServerEvent('kvl-terzi:GetExp')
end)


RegisterNetEvent('kvl-terzi:GetExp')
AddEventHandler('kvl-terzi:GetExp', function(data)
    xps = data
end)
          ...
          for i = 1, #KVL do
             if #(GetEntityCoords(player) - KVL[i].coords) <= 2 then -- 2'yi arttırır iseniz yazıları görüş mesafesi artar
                  ...
                  if KVL[i].type == "collecting" then
                    text = "~r~[E]~s~ Yün Topla"
                    if IsControlJustReleased(0, 38) then;
                        TriggerEvent("mythic_progbar:client:progress", {
                            name = "collecting",
                            duration = 5000,
                            label = 'Yün topluyorsun...',
                            useWhileDead = false,
                            canCancel = false,
                             controlDisables = {
                  ...
                  elseif KVL[i].type == "fabric" then
                      text = "~r~[E]~s~ Kumas Uret"
                      if IsControlJustReleased(0, 38) then;
                        TriggerEvent("mythic_progbar:client:progress", {
                            name = "fabric",
                            duration = 5000,
                            label = 'Kumaş üretiyorsun',
                            useWhileDead = false,
                            canCancel = false,
                             controlDisables = {
                                disableMovement = true,
                  ...
                  elseif KVL[i].type == "gomlekdikme" then
                     text = "~r~[E]~s~ Gömlek Dik"
                     if IsControlJustReleased(0, 38) then;
                        TriggerEvent("mythic_progbar:client:progress", {
                            name = "gomlekdikme",
                            duration = 5000,
                            label = 'Gömlek dikiyorsun',
                            useWhileDead = false,
                            canCancel = false,
                             controlDisables = {
                                disableMovement = true,
                  ...
                  elseif KVL[i].type == "pantolondikme" then
                    text = "~r~[E]~s~ Pantolon Dik"
                    if IsControlJustReleased(0, 46) then;
                        TriggerEvent("mythic_progbar:client:progress", {
                            name = "pantolondikme",
                            duration = 5000,
                            label = 'Pantolon dikiyorsun',
                            useWhileDead = false,
                            canCancel = false,
                             controlDisables = {
                                disableMovement = true,
                  ...
                  elseif KVL[i].type == "tshirtdikme" then
                    text = "~r~[E]~s~ Tshirt Dik"
                    if IsControlJustReleased(0, 46) then;
                        TriggerEvent("mythic_progbar:client:progress", {
                            name = "tshirtdikme",
                            duration = 5000,
                            label = 'Tshirt dikiyorsun',
                            useWhileDead = false,
                            canCancel = false,
                             controlDisables = {
                                disableMovement = true,
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if distance < 5 then
            sleep = 7
            DrawText3D(Config.NPCText.x, Config.NPCText.y, Config.NPCText.z, '[E] - Finn')
...
function OpenMenu()
    ...
    Citizen.Wait(100)
         if xps >= 0 and xps < 2 then -- ? Eğer xpim 0'dan büyük 2 den küçükse aşağıdaki menüyü getir.
            el = {
                {label = 'Tshirt $1000', value = 'tshirt', price = 1000},
                {label = 'Pantolon $1000', value = 'pantolon', price = 1000},
                {label = 'Gömlek $1000', value = 'gomlek', price = 1000},
            }
         elseif xps >= 2 and xps < 5 then
            el = {
                {label = 'Tshirt $2000', value = 'tshirt', price = 2000},
                {label = 'Pantolon $2000', value = 'pantolon', price = 2000},
                {label = 'Gömlek $2000', value = 'gomlek', price = 2000},
            }
         elseif xps >= 5 and xps < 10 then
            el = {
                {label = 'Tshirt $3000', value = 'tshirt', price = 2000},
                {label = 'Pantolon $3000', value = 'pantolon', price = 2000},
                {label = 'Gömlek $3000', value = 'gomlek', price = 2000},
            }
         end
