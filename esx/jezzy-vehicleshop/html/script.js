...
function menuVehicle(event) {
    ...
    $.post('http://jezzy-vehicleshop/menuSelected', JSON.stringify({menuId: categoryVehicleSelected.toLowerCase()}));
}

function testDrive(){
    $.post('http://jezzy-vehicleshop/TestDrive', JSON.stringify({vehicleModel: vehicleSelected.modelcar}));    
...
function buyVehicle(option) {
    ...
    switch(option){
        case 'cancel':
            break;
        case 'confirm':
            $.post('http://jezzy-vehicleshop/Buy', JSON.stringify(vehicleSelected));
            break;
    }   
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESC
            $.post('http://jezzy-vehicleshop/Close');
            $('body').css("display","none");
        ...
        case 17: // TAB
            break;
        case 68: // LEFT A
            $.post('http://jezzy-vehicleshop/rotate', JSON.stringify({key: "left"}))
            break;            
        case 65: // RIGHT D
            $.post('http://jezzy-vehicleshop/rotate', JSON.stringify({key: "right"}))
            break;
        case 39: 
...
$(document).on('keydown', function(ev) {
    ...
    if (_key) {
        $.post('http://jezzy-vehicleshop/rotate', JSON.stringify({key: _key}))
    }
    ...
    switch(event.which) {        
        ...
        case 1:
            ...
            if(!itemDisabled) {
                ...
                $.post("http://jezzy-vehicleshop/SpawnVehicle", JSON.stringify({ modelcar: dataCar[9].outerText, price: dataCar[7].outerText }));
...
//         $.post("http://jezzy-vehicleshop/RGBVehicle", JSON.stringify({primary: true, R: R, G: G, B: B}));
//     } else {
//         $.post("http://jezzy-vehicleshop/RGBVehicle", JSON.stringify({primary: false, R: R, G: G, B: B}));
//     }
