...
mainMenu = NativeUI.CreateMenu("IMPERIAL", "", menuPosition["x"], menuPosition["y"], Menuthing, Menuthing)
...
function AddEmoteMenu(menu)
    ...
    if not Config.SqlKeybinding then
      ...
      keyinfo =  NativeUI.CreateItem(Config.Languages[lang]['keybinds'], Config.Languages[lang]['keybindsinfo'].." /emotebind [~y~num4-9~w~] [~g~animasyonadı~w~]")
...
function AddInfoMenu(menu)
    if not UpdateAvailable then
      infomenu = _menuPool:AddSubMenu(menu, Config.Languages[lang]['infoupdate'], "(Destek bilgileri)", "", Menuthing, Menuthing)
    else
    ...
    u170 = NativeUI.CreateItem("Developer", "~b~Discord:~s~ BrnOrkun#778")
