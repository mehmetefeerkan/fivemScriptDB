...
RegisterNetEvent('dd_robvendor:onPedDeath')
AddEventHandler('dd_robvendor:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
end)

RegisterNetEvent('dd_robvendor:msgPolice')
AddEventHandler('dd_robvendor:msgPolice', function(store, robber)
    ...
    while true do
        ...
        exports['dd_utils']:ShowXn('show','darkblue','[E] - Set way point | [BACKSPACE] - Close')
...
RegisterNetEvent('dd_robvendor:removePickup')
AddEventHandler('dd_robvendor:removePickup', function(bank)
    for i = 1, #objects do 
...
RegisterNetEvent('dd_robvendor:robberyOver')
AddEventHandler('dd_robvendor:robberyOver', function()
    robbing = false
end)

RegisterNetEvent('dd_robvendor:talk')
AddEventHandler('dd_robvendor:talk', function(store, text, time)
    robbing = false
...
RegisterNetEvent('dd_robvendor:rob')
AddEventHandler('dd_robvendor:rob', function(i)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                ...
                                TriggerServerEvent('dd_robvendor:pickUp', i)
                                break
                            end
                        else
                            break
...
RegisterNetEvent('dd_robvendor:resetStore')
AddEventHandler('dd_robvendor:resetStore', function(i)
...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #peds do
                if IsPedDeadOrDying(peds[i]) then
                    TriggerServerEvent('dd_robvendor:pedDead', i)
                end
            end
            Wait(5000)
        end
            ...
            if IsPlayerFreeAiming(PlayerId()) then
                for i = 1, #peds do
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            local canRob = nil
                            ESX.TriggerServerCallback('dd_robvendor:canRob', function(cb)
                                canRob = cb
                            end, i)
                            while canRob == nil do
                                Wait(0)
                            ...
                            if canRob == true then
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 then
                                    if not IsPedDeadOrDying(peds[i]) then
                                        TriggerServerEvent('dd_robvendor:rob', i)
                                        while robbing do Wait(0) if IsPedDeadOrDying(peds[i]) then robbing = false end end
                                    end
                                else
                                    ClearPedTasks(peds[i])
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                TriggerEvent('dd_robvendor:talk', i, '~g~*' .. Translation[Config.Locale]['shopkeeper'] .. '* ~w~' .. Translation[Config.Locale]['robbed'], 5)
                                Wait(2500)
                            end
                        end
                    end
