...
blipshs = {}
RegisterNetEvent('kvl:berberblip')
AddEventHandler('kvl:berberblip',function()
    if not bliptrue then
        bliptrue = true
        for k, v in pairs(Config.Zones3) do
            blipolustur(v, 71, 51, 0.5, 'Berber') -- 75 = Blip 4 = color 0.6 = blip scale oluyor
        end
        ESX.ShowNotification('Berber blipleri açıldı')
    else
        bliptrue = false
        for k, v in pairs(blipshs) do
            RemoveBlip(v)
        end
        ESX.ShowNotification('Berber blipleri kapatıldı')
    end
end)
