...
window.onload = function(e) {
    ...
    $('#ghizg-on').on('change',function(){
        $.post('https://qb-hud/fh4speed', JSON.stringify({status: this.checked}));
        if (this.checked) {
    ...
    $('#pusula-on').on('change',function(){
    	if(this.checked){
            ...
            $.post('https://qb-hud/set-compass', JSON.stringify({compassOn: true}));
    	}
        else{
            if (pusulaAlt) {
            ...
            $.post('https://qb-hud/set-compass', JSON.stringify({compassOn: false}));
        }
    });
