...
function ChopVehicle()
    ...
    elseif GetGameTimer() - Timer > Config.CooldownMinutes * 60000 then
        ...
        if TotalTime >= 0 then
            ESX.ShowNotification(TotalTime.. " Dakika sonra gel")
        elseif TotalTime <= 0 then
            ESX.ShowNotification(_U('Bekleme Süresi: '.. TotalTime))
        end
    end
end
...
function VehiclePartsRemoval()
    ...
    if bonnet ~= -1 then
        if ChoppingInProgress == true then
            ESX.ShowNotification("Kaput Açılıyor")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 4, false, false)
        end
        Citizen.Wait(2000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Kaput Sökülüyor")
            Citizen.Wait(Config.RemovePart)
    ...
    if boot ~= -1 then
        if ChoppingInProgress == true then
            ESX.ShowNotification("Bagaj Açılıyor")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 5, false, false)
        end
        Citizen.Wait(2000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Bagaj Sökülüyor")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false),5, true)
        end
    end
    Citizen.Wait(3000)
    ESX.ShowNotification("Araç Ortadan Kaldırılıyor..")
    Citizen.Wait(Config.RemovePart)
    if ChoppingInProgress == true then
        local vehicle =  GetVehiclePedIsIn( ped, false )
        if vehicle then
            ...
            ESX.TriggerServerCallback('Lenzh_chopshop:OwnedCar', function(owner)
                if owner then
                    ESX.ShowNotification("Kendi Aracını Başarılı Bir Şekilde Parçaladın...", false, true, g)
                    DeleteVehicle(vehicle)
                else
                    ESX.ShowNotification("Araç Başarılı Bir Şekilde Parçalandı...", false, true, g)
                   DeleteVehicle(vehicle)
                end
            end, vehPlate)
        end
