...
RegisterNetEvent('balik:antibug')
AddEventHandler('balik:antibug', function()
	if balikbug == false then
		TryToFish()
		Citizen.Wait(1000)
		balikbug = true
	else
		TriggerEvent('notification', 'Zaten balık tutuyorsun', 2)
	end
end)
...
TryToFish = function()
    ...
    }) then balikbug = false return ESX.ShowNotification("Balık tutmak için envanterinde olta ve yem olmalı.") end
...
CastBait = function(rodHandle, castLocation)
    ...
    if caughtFish then
        ESX.TriggerServerCallback("james_fishing:receiveFish", function(received)
            if received then
                ESX.ShowNotification("Bir balık yakaladın!")
            end
        end)
    else
...
SellFish = function()
    ...
    }) then return ESX.ShowNotification("Envanterinde balık yok.") end
