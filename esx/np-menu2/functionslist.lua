...
AddEventHandler('pw:handcuff', function()
    ...
    if distance <= 2.0 then
        if distance == -1 then
            exports['chubbsnotify']:DoHudText('error', 'Yakında kelepçeleyebileceğin biri yok', 4000)
        else
    ...
    if health == 0 then
        ...
        ClearPedTasks(playerPed)
            if GetEntityHealth(closestPlayerPed) == 0 then
                ...
                exports['chubbsnotify']:DoHudText('error', 'Yakında ölü biri yok', 4000)
            end
        end)
    end
end)
...
AddEventHandler('pw:menuimpound', function()
    ...
    if vehicle ~= nil then
        ...
        exports['chubbsnotify']:DoHudText('success', 'Araba çekildi')
        Citizen.Wait(100)
    else
        exports['chubbsnotify']:DoHudText('error', 'Yakında araba yok')
    end
end)
...
AddEventHandler('pw:open', function()
    ...
    if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
        ...
        exports['chubbsnotify']:DoHudText('inform', 'Aracın kilidini açtın', 4000)
    else
        exports['chubbsnotify']:DoHudText('error', 'Yakında araba yok', 4000)
    end
end)
...
AddEventHandler('pw:mechrepair', function()
    ...
    if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
        ...
        exports['chubbsnotify']:DoHudText('inform', 'Aracı Tamir ettin', 4000)
    else
        exports['chubbsnotify']:DoHudText('error', 'Yakında araba yok', 4000)
    end
end)
...
AddEventHandler('pw:mechclean', function()
    ...
    if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
        ...
        exports['chubbsnotify']:DoHudText('inform', 'Aracı temizledin', 4000)
    else
        exports['chubbsnotify']:DoHudText('error', 'Yakında araba yok', 4000)
    end
end)
...
local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
			if closestPlayer == -1 or closestDistance > 3.0 then
				exports['chubbsnotify']:DoHudText('error', 'Yakında Fatura Kesilecek Kimse Yok')
			return
end
ESX.UI.Menu.Open(
	...
	local amount = tonumber(data2.value)
	    if amount == nil then
			exports['chubbsnotify']:DoHudText('inform', (_U('invalid_amount')))
		else
...
ESX.UI.Menu.Open(
	...
	local amount = tonumber(data2.value)
	    if amount == nil then
			exports['chubbsnotify']:DoHudText('inform', 'Geçersiz Miktar')
		else
...
AddEventHandler('pw:emsRevive', function()
    ...
    if closestPlayer == -1 or closestDistance > 1.0 then
        exports['chubbsnotify']:DoHudText('error', 'Yakında oyuncu yok')
    else
        ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
            if quantity > 0 then
                ...
                if IsPedDeadOrDying(closestPlayerPed, 1) then
                    ...
                    exports['chubbsnotify']:DoHudText('error', 'Kişinin kritik durumu yok')
                end
            else
                exports['chubbsnotify']:DoHudText('error', 'Medikitin yok')
            end
        ...
        ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
            if quantity > 0 then
                ...
                if health > 0 then
                    ...
                    exports['chubbsnotify']:DoHudText('error', 'Kişinin kötü durumu yok')
                end
            else
                exports['chubbsnotify']:DoHudText('error', 'Bandajın yok')
            end
        end, 'bandage')
    end
end)
...
AddEventHandler('FlipVehicle', function()
    ...
    if IsPedInAnyVehicle(ped, false) then vehicle = GetVehiclePedIsIn(ped, false) else vehicle = GetClosestVehicle(coords.x, coords.y, coords.z, 5.0, 0, 71) end
        ...
        exports['chubbsnotify']:DoHudText('error', 'Araba zaten düz durumda', 7000)
    end
end)
