		...
		if ESX.PlayerData.job and (ESX.PlayerData.job.name == 'police') then
			for k,v in pairs(Config.PoliceGarage) do
				...
				if distance <= 20.0 then
					...
					if distance <= 3 then
						DrawText3Ds(v[1], v[2], pos.z, "~g~[E]~s~ Tuşuna Tıkla ~y~Polis Garajı~s~")
					...
					if distance <= 20.0 then
						...
						if distance <= 3 then
							if not menuData.open then
								DrawText3Ds(v[1], v[2], pos.z, " ~g~[E]~s~ Tuşuna Tıkla ~y~Arac Ekstrası~s~")
...
PoliceGarage = function()
	local elements = {
		...
		{ label = "Get Vehicle", action = "get_vehicle" },
	}
	...
	function(data, menu)
		...
		elseif data.current.action == 'store_vehicle' then
			...
			if dist < 3 then
				DeleteEntity(veh)
				ESX.ShowNotification("~b~Vehicle~s~ was parked!")
			else
				ESX.ShowNotification("No ~b~vehicle~s~ nearby!")
			end
		end
...
VehicleMenu = function()
	...
	function(data, menu)
		...
		ESX.ShowNotification("You took out a ~b~Vehicle~s~ from ~y~Police Garage~s~!")
...
function VehicleLoadTimer(modelHash)
	...
	if not HasModelLoaded(modelHash) then
		...
		while not HasModelLoaded(modelHash) do
			...
			drawLoadingText("Wait for vehicle to spawn", 255, 255, 255, 255)
		end
	end
end
	...
	for k,v in pairs(Config.PoliceGarage) do 
		if IsPedInAnyVehicle(ped, true) then
			...
			if GetDistanceBetweenCoords(GetEntityCoords(ped), v[1], v[2], v[3], true) <= Config.Distance then
				ESX.ShowNotification("Your ~b~vehicle~s~ is being ~y~repaired~s~, please wait!")
				FreezeEntityPosition(veh, true)
				exports['progressBars']:startUI(5000, "REPAIRING VEHICLE")
				Citizen.Wait(5000)
				ESX.ShowNotification("Your ~b~vehicle~s~ has been ~y~repaired~s~!")
				SetVehicleFixed(veh)
...
function policeClean()
	...
	for k,v in pairs(Config.PoliceGarage) do 
		if IsPedInAnyVehicle(ped, true) then
			local veh = GetVehiclePedIsIn(ped, false)
			if GetDistanceBetweenCoords(GetEntityCoords(ped), v[1], v[2], v[3], true) <= Config.Distance then
				ESX.ShowNotification("Your ~b~vehicle~s~ is being ~y~cleaned~s~, please wait!")
				FreezeEntityPosition(veh, true)
				exports['progressBars']:startUI(3500, "CLEANING VEHICLE")
				Citizen.Wait(3500)
				ESX.ShowNotification("Your ~b~vehicle~s~ has been ~y~cleaned~s~!")
				SetVehicleDirtLevel(veh, 0.1)
				FreezeEntityPosition(veh, false)
			end
		end
