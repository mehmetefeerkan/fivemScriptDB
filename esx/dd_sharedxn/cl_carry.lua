...
-- function droppedHimHer()
-- 	Citizen.CreateThread(function()
-- 		while true do
-- 			exports['dd_utils']:ShowXn('show','green','Press C to drop')
-- 			-- HelpNotification("~INPUT_VEH_DUCK~ to drop the body")
-- 			if IsControlJustPressed(1, 324) then -- C
-- 				print("Pressed C")
-- 				-- DragClosest()
-- 				TriggerEvent("SharedXn:Carry")
...
-- RegisterNetEvent("SharedXn:Carry")
-- AddEventHandler("SharedXn:Carry", function()
...
AddEventHandler('CarryPeople:syncMe', function(animationLib, animation,length,controlFlag,animFlag)
	...
	Wait(500)
	if controlFlag == nil then controlFlag = 0 end
	TaskPlayAnim(playerPed, animationLib, animation, 3.0, -8.0, length, controlFlag, 0, false, false, false)
	...
	while true do
		if carryingBackInProgress then 
			while not IsEntityPlayingAnim(GetPlayerPed(-1), carryAnimDictPlaying, carryAnimNamePlaying, 3) do
				TaskPlayAnim(GetPlayerPed(-1), carryAnimDictPlaying, carryAnimNamePlaying, 3.0, -8.0, 100000, carryControlFlagPlaying, 0, false, false, false)
				Citizen.Wait(0)
			end
		end
		Wait(0)
