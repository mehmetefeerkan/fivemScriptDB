...
RegisterNetEvent('mythic_progbar:client:ToggleBusyness')
AddEventHandler('mythic_progbar:client:ToggleBusyness', function(bool)
    isDoingAction = bool
end)
...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            exports["mythic_notify"]:SendAlert("error", "Zaten birsey üzerinde calisiyorsun!", 4000)
        end
    else
        exports["mythic_notify"]:SendAlert("error", "Islem gerceklestirilemez!", 4000)
    end
end
