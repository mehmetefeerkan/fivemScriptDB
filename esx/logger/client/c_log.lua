    ...
    while true do
        ...
        if IsEntityDead(playerPed) and not alreadyDead then
            ...
            if checkArray (Melee, death) then
                TriggerServerEvent('Oldurulen', killername .. " Yumruklayarak " .. playerName .. "öldürdü")
            elseif checkArray (Bullet, death) then
                TriggerServerEvent('Oldurulen', killername .. " Silahla vurarak " .. playerName .. "öldürdü")
            elseif checkArray (Knife, death) then
                TriggerServerEvent('Oldurulen', killername .. " Bıçaklayarak " .. playerName .. "öldürdü")
           -- elseif checkArray (Car, death) then
            --    TriggerServerEvent('Oldurulen', killername .. " Arabayla ezerek " .. playerName .. "öldürdü")
            elseif checkArray (Animal, death) then
                TriggerServerEvent('KendiOlen', "Hayvan tarafından öldürüldü")
            elseif checkArray (FallDamage, death) then
                TriggerServerEvent('KendiOlen', "Düşerek öldü")
            elseif checkArray (Explosion, death) then
                TriggerServerEvent('KendiOlen', "Patlama yüzünden öldü")
            elseif checkArray (Gas, death) then
                TriggerServerEvent('KendiOlen', "Gaz yüzünden öldü")
            elseif checkArray (Burn, death) then
                TriggerServerEvent('KendiOlen', "Yanarak öldü")
            elseif checkArray (Drown, death) then
                TriggerServerEvent('KendiOlen', "Boğularak öldü")
            else
                TriggerServerEvent('KendiOlen', "Bilinmiyor")
                --print(death)
            end
