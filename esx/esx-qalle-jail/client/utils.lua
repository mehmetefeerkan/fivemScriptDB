RegisterCommand("hapismenu", function(source, args)

	if PlayerData.job.name == "police" then
		OpenJailMenu()
	else
		-- exports['mythic_notify']:SendAlert('error', 'Polis değilsin, kimseyi hapse tıkamazsın')
		TriggerClientEvent('notification', source, 'Polis değilsin, kimseyi hapse tıkamazsın', 2)
	end
end)
...
function TeleportPlayer(pos)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Boiling Broke" or action == "Güvenlik" then

				if PlayerData.job.name ~= "police" then
					-- exports['mythic_notify']:SendAlert('error', 'Buraya girmek için anahtarın yok')
					TriggerClientEvent('notification', source, 'Buraya girmek için anahtarın yok', 2)
					return
				end
			end
...
Citizen.CreateThread(function()
    ...
    AddTextComponentString('Federal Hapishane')
