...
--[[ Main Event Handlers ]]--
    ...
    AddEventHandler("K9:OpenMenu", function(pedRestriction, pedList)
        if pedRestriction then
            if CheckPedRestriction(GetLocalPed(), pedList) then
                ...
                TriggerEvent("pNotify:SendNotification", {text = "Zmień model postaci, aby używać K9."})
            end
        else
            EnableMenu()
        end
    ...
    AddEventHandler("K9:IdentifierRestricted", function()
        --TriggerEvent("pNotify:SendNotification", {text = "~r~Nie masz uprawnień!"})
    end)
	...
	TriggerEvent('notification', 'Sadece K9 Çağırabilirsin!', 2)
	else
        if spawned_ped == nil then
		TriggerEvent('notification', 'Pet Çağırıldı!', 1)
		...
		TriggerEvent('notification', 'Pet Gönderildi!', 1)
            local has_control = false
            RequestNetworkControl(function(cb)
                has_control = cb
            end)
...
AddEventHandler('esx:setJob', function(job)
	...
	if PlayerData.job and PlayerData.job.name == 'police' then
        if spawned_ped == nil then
		TriggerEvent('notification', 'K9 Çağırıldı', 1)
            local ped = GetHashKey(model)
            RequestModel(ped)
            while not HasModelLoaded(ped) do
                Citizen.Wait(1)
		...
		TriggerEvent('notification', 'K9 Gönderildi', 1)
            local has_control = false
            RequestNetworkControl(function(cb)
                has_control = cb
            end)
    ...
    AddEventHandler("K9:ToggleVehicle", function(isRestricted, vehList)
        if not searching then
            if IsPedInAnyVehicle(spawned_ped, false) then
				...
				ExecuteCommand('e parmakaşağı')
								exports["t0sic_loadingbar"]:StartDelayedFunction("K9 araçtan iniyor", 1500, function()
								SetEntityCoords(spawned_ped, currentPos.x, currentPos.y, currentPos.z, false, false, false, true)
								TriggerEvent('notification', 'K9 araçtan indi.', 1)
								end)
				Citizen.Wait(18)
    ...
    AddEventHandler("K9:SearchVehicle", function(items, openDoors)
        ...
        if vehicle ~= 0 and not searching then
            ...
            TriggerEvent("pNotify:SendNotification", {text = k9_name .. " zaczął szukać..."})
            ...
            TriggerEvent("pNotify:SendNotification", {text = k9_name .. " znalazł [ " .. tostring(table.concat(stringified_table, ", ")) .. " ]."})
            searching = false
        end
    end)
                ...
                if distance ~= -1 then
                    if distance <= 3.0 then
                        ...
                        if not IsPedInCombat(spawned_ped, playerPed) then
							...
							exports["t0sic_loadingbar"]:StartDelayedFunction("K9 saldırıyor", 3000, function()
