...
local PlayerData		= {}
local pedIsTryingTolockpickaracVehicle  = false
...
--                MAIN FUNCTION                 --
--//////////////////////////////////////////////--
RegisterNetEvent('esx_lockpickarac:onUse')
AddEventHandler('esx_lockpickarac:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			if Config.IgnoreAbort then
				TriggerServerEvent('esx_lockpickarac:removeKit')
			end
	    ...
	    RequestAnimDict('anim@amb@clubhouse@tutorial@bkr_tut_ig3@')
            ...
            TriggerEvent("mythic_progressbar:client:progress", {
                name = "lockpickaracing",
                duration = Config.LockTime * 1000,
                label = "Maymuncuk Uygulanıyor..",
                useWhileDead = false,
                canCancel = true,
                controlDisables = {
                ...
                if not status then
                    ...
                    exports.pNotify:SendNotification({
                        text = (_U('lockpickaraced_successful')), 
                        type = "success", 
                        timeout = 1000, 
                        layout = "centerRight", 
                        queue = "right",
			...
			Citizen.CreateThread(function()
				...
				CurrentAction = 'lockpickarac'
				local chance =	lockpickaracchance()
				if chance == true then
					if Config.CallCops then
						...
						if randomReport == Config.CallCopsPercent then
							TriggerServerEvent('esx_lockpickarac:Notify')
						end
                    end
		...
		Citizen.CreateThread(function()
			...
			if CurrentAction ~= nil then
				...
				if IsControlJustPressed(0, 178) then
					...
					ESX.ShowNotification(_U('aborted_lockpickaracing'))
					CurrentAction = nil
				end
			end
		end)
			...
			local lock = GetVehicleDoorLockStatus(veh)
				...
				if ((lock == 7) or (pedd ~= 0 )) then
					if has_value(Config.job_whitelist, xPlayer.job.name) then
						TriggerServerEvent('esx_lockpickarac:setVehicleDoorsForEveryone', {veh, 1, plate})
					else
						if not lucky or blacklisted then
							TriggerServerEvent('esx_lockpickarac:setVehicleDoorsForEveryone', {veh, 2, plate})
						else
							TriggerServerEvent('esx_lockpickarac:setVehicleDoorsForEveryone', {veh, 1, plate})
						end
					end
...
RegisterNetEvent('esx_lockpickarac:setVehicleDoors')
AddEventHandler('esx_lockpickarac:setVehicleDoors', function(veh, doors)
...
RegisterNetEvent('esx_lockpickarac:outlawLockNotify')
AddEventHandler('esx_lockpickarac:outlawLockNotify', function(alert)
	if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
		TriggerEvent('esx_lockpickarac:notify2')
		PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
    end
end)

RegisterNetEvent("esx_lockpickarac:notify2")
AddEventHandler("esx_lockpickarac:notify2", function(msg, target)
	ESX.ShowAdvancedNotification(_U('911Call'), _U('911lockpickarac'), _U('Lockcall'), 'CHAR_CALL911', 7)
end)
--//////////////////////////////////////////////--
--                   NETWORK                    --
--//////////////////////////////////////////////--
Citizen.CreateThread(function()
    while true do
        ...
        if NetworkIsSessionStarted() then
            ...
            DecorSetInt(PlayerPedId(), "IsLockOutlaw", 1)
            return
        end
    end
end)
...
Citizen.CreateThread( function()
    while true do
        ...
        if pedIsTryingTolockpickaracVehicle then
            DecorSetInt(PlayerPedId(), "IsLockOutlaw", 2)
            ...
            elseif PlayerData.job ~= nil and PlayerData.job.name == 'police' and showcopsmisbehave then
                ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
                    ...
                    TriggerServerEvent('esx_lockpickarac:InProgressPos', plyPos.x, plyPos.y, plyPos.z)
                    if s2 == 0 then
                        TriggerServerEvent('esx_lockpickarac:InProgressS1', street1, sex)
                    elseif s2 ~= 0 then
                        TriggerServerEvent('esx_lockpickarac:InProgress', street1, street2, sex)
                    end
                end)
                Wait(3000)
                pedIsTryingTolockpickaracVehicle = false
            else
                ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
                    local sex = nil
                    if skin.sex == 0 then
...
--              SUSPECT LOCATION                --
--//////////////////////////////////////////////--
RegisterNetEvent('esx_lockpickarac:location')
AddEventHandler('esx_lockpickarac:location', function(tx, ty, tz)
...
--               lockpickarac CHECK                 --
--//////////////////////////////////////////////--
RegisterNetEvent('esx_lockpickarac:Enable')
AddEventHandler('esx_lockpickarac:Enable', function()
	pedIsTryingTolockpickaracVehicle = true
end)
