...
--[[ Main Event Handlers ]]--
        ...
        if spawned_ped ~= nil then
            if not following then
                ...
                if has_control then
                    ...
                    following = false
                    attacking = 
                    exports['mythic_notify']:DoHudText('error', 'K9 Olduğu yerde durdu!')
                end
            end
        end
    end)
    ...
    AddEventHandler("K9:ToggleVehicle", function(isRestricted, vehList)
        if not searching then
            if IsPedInAnyVehicle(spawned_ped, false) then
                ...
                exports['mythic_notify']:DoHudText('error', 'K9 Aracın dışına çıktı!')
            else
                    ...
                    if door ~= false then
                        if isRestricted then
                            if CheckVehicleRestriction(vehicle, vehList) then
                                ...
                                exports['mythic_notify']:DoHudText('success', 'K9 Aracın içine bindi!')
                            end
                        else
                            TaskEnterVehicle(spawned_ped, vehicle, -1, door, 1.0, 1, 0)
                            exports['mythic_notify']:DoHudText('success', 'K9 Aracın içine bindi!')
    ...
    AddEventHandler("K9:SearchVehicle", function(items, openDoors)
        ...
        if vehicle ~= 0 and not searching then
            ...
            exports['mythic_notify']:DoHudText('inform', 'K9 Aracı tamamen aramaya başladı!')
            ...
            exports['mythic_notify']:DoHudText('error', 'K9 un buldukları: ' .. tostring(table.concat(stringified_table)))
            searching = false
        end
    end)
    ...
    AddEventHandler("k9:searchplayer", function(items)
        ...
        if closestPlayer ~= -1 and not searching and closestDistance <= 3.0 then
            ...
            exports['mythic_notify']:DoHudText('inform', 'K9 kişiyi aramaya başladı!')
