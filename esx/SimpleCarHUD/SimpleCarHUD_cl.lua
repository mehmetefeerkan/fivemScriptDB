...
local seatbeltColorOn = {160, 255, 160, 0}     -- Color used when seatbelt is on
local seatbeltColorOff = {255, 96, 96, 0}      -- Color used when seatbelt is off

-- CRUISE CONTROL PARAMETERS
local cruiseInput = 29                     -- Toggle cruise on/off with CAPSLOCK or A button (controller)
local cruiseColorOn = {160, 255, 160, 0}       -- Color used when seatbelt is on
local cruiseColorOff = {255, 255, 255, 0}      -- Color used when seatbelt is off
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if pedInVeh or locationAlwaysOn then
            ...
            drawTxt(timeText, 4, locationColorText, 0.40, screenPosX, screenPosY + 0.018)
            ...
            if pedInVeh and GetIsVehicleEngineRunning(vehicle) and vehicleClass ~= 13 then
                ...
                if GetPedInVehicleSeat(vehicle, -1) then
                    if ShouldUseMetricMeasurements() then
                        ...
                        drawTxt(("%.3d"):format(math.ceil(speed)), 2, speedColor, 0.59, screenPosX + 0.000, screenPosY + 0.040)
                        drawTxt("KMH", 2, speedColorText, 0.3, screenPosX + 0.025, screenPosY + 0.055)
                    else
                    ...
                    drawTxt(("%.3d"):format(math.ceil(currentFuel)), 4, fuelColor, 0.59, screenPosX + 0.055, screenPosY + 0.040)
                    drawTxt("BENZIN", 2, fuelColorText, 0.3, screenPosX + 0.080, screenPosY + 0.055)
                else
                    if ShouldUseMetricMeasurements() then
                        -- Get vehicle speed in KPH and draw speedometer
                        local speed = currSpeed*3.6
