			...
			if PlayerData.job ~= nil and PlayerData.job.name ~= 'unemployed' and PlayerData.job.name == "police" then
				if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
					...
					if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.5 then
						DrawText3D(v.Pos.x, v.Pos.y, v.Pos.z+0.9, "~w~[~g~E~w~] Kameraları aç.", 0.80)
					end
				end
			end
		end
			...
			if Config.Hacking then
				if PlayerData.job ~= nil and PlayerData.job.name ~= 'unemployed' and PlayerData.job.name == "police" then
					...
					if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
						...
						if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.5 then
							DrawText3D(v.Pos.x, v.Pos.y, v.Pos.z+0.9, "~w~[~g~E~w~] Kameraları hacklemeye başla.", 0.80)
						end
					end
				end
			end
			...
			if Config.Hacking then
				if PlayerData.job ~= nil and PlayerData.job.name ~= 'unemployed' and PlayerData.job.name == "police" then
					if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
						...
						if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.5 then
							DrawText3D(v.Pos.x, v.Pos.y, v.Pos.z+0.9, "~w~[~g~E~w~] Virüsleri kaldır.", 0.80)
						end
					end
				end
			end
...
function mycbpolice(success, timeRemaining)
	if success then
		...
		TriggerServerEvent('esx_phone:send', 'police', 'POLICE ALARM: Birisi bilgisayarları hacklemeye çalışıyor, çabuk karakola koşun.', false, false)
	end
end
...
function mycbbank(success, timeRemaining)
	if success then
		...
		Citizen.Wait(500)
		PlaySoundFrontend(-1, "TextHit", "WastedSounds", 1)
		TriggerServerEvent('esx_phone:send', 'police', 'BANK: Yardım edin! Birisi bankadaki bilgisayarları hacklemeye çalışıyor, çabuk gelin ya da güvenlik kamerasından kontrol edin.', false, false)
	end
end
