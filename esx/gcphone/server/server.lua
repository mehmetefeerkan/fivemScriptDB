...
MySQL.ready(function ()
   ...
   -- MySQL.Async.execute('DELETE FROM phone_messages WHERE transmitter = \'vehicledealer\'')
end)
...
RegisterServerEvent('crew:allUpdate')
AddEventHandler('crew:allUpdate', function()
    ...
    local identifier = xPlayer.identifier
    local num = getNumberPhone(identifier)
    TriggerClientEvent("crew:updatePhone", sourcePlayer, num)
    TriggerClientEvent("gcPhone:contactList", sourcePlayer, getContacts(identifier))
    TriggerClientEvent("gcPhone:allMessage", sourcePlayer, getMessages(identifier))
    --TriggerClientEvent('gcPhone:getBourse', sourcePlayer, getBourse())
    sendHistoriqueCall(sourcePlayer, num)
...
AddEventHandler('gcPhone:sendMessage2', function(phoneNumber, message)
    local sourcePlayer = 'Anonymous'
    xPlayer = ESX.GetPlayerFromId(source)
    local identifier = xPlayer.identifier
    addMessage(sourcePlayer, identifier, phoneNumber, message)
end)
...
AddEventHandler('gcPhone:getHistoriqueCall', function()
    ...
    local srcPhone = xPlayer.identifier
    sendHistoriqueCall(sourcePlayer, num)
end)
...
RegisterNetEvent('gcPhone:removeItem')
AddEventHandler('gcPhone:removeItem', function(item)
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeInventoryItem(item, 1)
end)

RegisterNetEvent('gcPhone:removeItem2')
AddEventHandler('gcPhone:removeItem2', function(item)
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeInventoryItem(item, 12)
end)