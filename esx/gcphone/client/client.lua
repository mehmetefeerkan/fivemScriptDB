...
AddEventHandler('esx:playerLoaded', function()
  TriggerServerEvent('crew:allUpdate')
end)
  ...
  if GetPlayerServerId(PlayerId()) == tonumber(phoneId) then
    ESX.TriggerServerCallback('crew-phone:phone-check', function(durum)
      if durum ~= nil then
        ...
        exports['okokNotify']:Alert("", "Incoming Call. . . .", 3000, 'info')
      --   exports.pNotify:SendNotification({
      --     text = "Incoming Call. . . .", 
      --     type = "info", 
      --     timeout = math.random(4000, 8000), 
      --     layout = "bottomCenter", 
      --     queue = "left"
      -- })
        --exports['okokNotify']:Alert("Phone", "Phone is Ringing. . .", 5000, 'inform')
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    --exports['co_notify']:SendNotify('MESSAGE', message.transmitter.." Message From.", message.message) --EKLENMESİ GEREKEN KOD!
    ESX.TriggerServerCallback('crew-phone:phone-check', function(durum)
      if durum ~= nil then
        exports['co_notify']:SendNotify('mesaj', message.transmitter.." Message From.", message.message) --EKLENMESİ GEREKEN KOD!
        local text = _U('new_one_message')
        if ShowNumberNotification == true then
          if message.transmitter == "police" or message.transmitter == "ambulance" or message.transmitter == "mechanic" or message.transmitter == "vehicledealer" or message.transmitter == "taxi" or message.transmitter == "syndicate" then
...
AddEventHandler("gcPhone:waitingCall", function(infoCall, initiator)
  ESX.TriggerServerCallback('crew-phone:phone-check', function(durum)
    if durum ~= nil then
  exports['co_notify']:SendNotify('arama', infoCall.transmitter_num, infoCall)
...
AddEventHandler("gcPhone:acceptCall", function(infoCall, initiator)
  if inCall == false and USE_RTC == false then
   ...
   -- NetworkSetTalkerProximity(0.0)
    ...
    exports['co_notify']:SendNotify('aramakabul')
  end
  if menuIsOpen == false then
    TooglePhone()
  end
...
AddEventHandler("gcPhone:rejectCall", function(infoCall)
  ...
  --PhonePlayText()
  exports['co_notify']:SendNotify('aramabitir')
      ...
      if notHacked then
        ESX.TriggerServerCallback('gcPhone:getItemAmount', function(quantity)
          ...
          ----------------------------------------------------------
            ...
            Citizen.Wait(100)
            TriggerServerEvent('gcPhone:removeItem2', 'mobile_load')
            exports['dd_core']:notify('inform', 'You\'ve used load.')
          ----------------------------------------------------------
        else
          exports['dd_core']:notify('error', 'You don\'t have enough load.')
        end
        end, 'mobile_load')
      end
    end)
      ...
      if notHacked then
        ESX.TriggerServerCallback('gcPhone:getItemAmount', function(quantity)
          if quantity >= 1 then
            ...
            TriggerServerEvent('gcPhone:sendMessage', data.phoneNumber, data.message)
            Citizen.Wait(100)
            TriggerServerEvent('gcPhone:removeItem', 'mobile_load')
            exports['dd_core']:notify('inform', 'You\'ve used load.')
            ----------------------------------------------------------
          else
            exports['dd_core']:notify('error', 'You don\'t have enough load.')
...
function TooglePhone() 
  ESX.TriggerServerCallback('crew-phone:phone-check', function(durum)
    if durum ~= nil then
      ...
      exports['okokNotify']:Alert("", "You need a Phone.", 3000, 'error')
    --   exports.pNotify:SendNotification({
    --     text = "You need a Phone.", 
    --     type = "info", 
...
RegisterNUICallback('takePhoto', function(data, cb)
  ...
  if hasFocus == true then
    ...
    elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
      exports['screenshot-basic']:requestScreenshotUpload("https://discord.com/api/webhooks/905174494031773756/qRN7aCrGfnbbOSn8-05gDEBOHYW8l-88tyKQ92zM1jQ_kRbo05PqfZBNmBT7gldmHFp6", data.field, function(data)
