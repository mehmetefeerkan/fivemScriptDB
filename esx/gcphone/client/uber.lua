...
RegisterNUICallback('crewPhone_uberTrigger', function()
    if uberBasladi then
		exports['dd_core']:notify('inform', 'Uber Delivery Search Closed', 5000)
		...
		exports['dd_core']:notify('success', 'Uber Delivery Search Activated [Please Wait]', 5000)
	...
	while true do
		...
		if uberBasladi and not paketNoktasi then
			...
			if uberBasladi then
				...
				exports['dd_core']:notify('success', 'New Delivery: '..Config.uberItems[uberChoosenItem]["name"]..' Delivery marked on the map', 5000)
		...
		if uberBasladi and uberChoosenAdress ~= nil then
			if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.uberDelivery[uberChoosenAdress]["x"], Config.uberDelivery[uberChoosenAdress]["y"], Config.uberDelivery[uberChoosenAdress]["z"], true ) < 1.2 and paketNoktasi and not kapiCalindi then
				Draw3DText2(Config.uberDelivery[uberChoosenAdress]["x"], Config.uberDelivery[uberChoosenAdress]["y"], Config.uberDelivery[uberChoosenAdress]["z"] + 0.3, tostring("~w~~g~[E]~w~ Knock Door"))
				...
				if(IsControlJustPressed(1, Keys["E"])) then
					ESX.TriggerServerCallback('crew-phone:item-check', function(qtty)
						if qtty > 0 then
							...
							exports['dd_core']:notify('inform', 'Hype! Custom Styling!', 5000)
							ESX.ShowNotification('You do not have: ' .. Config.uberItems[uberChoosenItem]["name"])
						end
...
function rewarduber()
	...
	exports['dd_core']:notify('success', 'Package Delivered $'.. ESX.Math.Round(price), 5000)
	
	if npcEvdemi == 1 then
		local bahsis = math.random(3,10)
		local toplamPara = price + bahsis
		TriggerServerEvent('esx_uber:pay', toplamPara)
		exports['dd_core']:notify('success', 'You get a tip $'.. bahsis, 5000)
