  ...
  local identifier =  ESX.GetPlayerFromId(src).identifier
        ...
        function(data)
            ...
            if count > data[1].count then
                ...
                TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text = "Sahip olduğundan daha fazlasını cıkaramazsın",type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
                else
      ...
      MySQL.Async.fetchAll(
        ...
        if data[1] == nil then
            ...
            elseif data ~= nil and data[1].price ~= Price then
                Wait(250)
                TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text = '~r~You already have a same item in your shop, ~r~but for ' .. data[1].price .. '. you put the price ' .. Price,type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
                Wait(250)
                TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text = "Eşyayı kaldır yeni fiyat koy yada aynı fiyata koy",type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
            end
        end)  
    end)
end)
  ...
  local xPlayer = ESX.GetPlayerFromId(src)
        ...
        'SELECT * FROM owned_shops WHERE ShopNumber = @Number',
        {
            ...
            if xPlayer.getMoney() < ItemCount * result[1].price then
                TriggerClientEvent("pNotify:SendNotification", src, {text = "Yeteri kadar paran yok",type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
            elseif ItemCount <= 0 then
                TriggerClientEvent("pNotify:SendNotification", src, {text = "Geçersiz miktar",type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
            else
                if xPlayer.canCarryItem(result[1].item, ItemCount) then
  ...
  local identifier = ESX.GetPlayerFromId(_source).identifier
    ...
    MySQL.Async.fetchAll('SELECT money FROM owned_shops WHERE ShopNumber = @ShopNumber AND identifier = @identifier',{['@ShopNumber'] = id,['@identifier'] = identifier,}, function(result)
        ...
        if result[1].money >= price * count then
            ...
            TriggerClientEvent("pNotify:SendNotification", _source, {text =  price * count .. ' $ Karşılığında' .. count .. ' adet ' .. label .. 'sipariş ettin'  ,type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
        else
            TriggerClientEvent("pNotify:SendNotification", _source, {text =  "Kasanda yeteri kadar paran yok",type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
        end
    end)
end)
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    if result[1].identifier == '0' then
        ...
        if xPlayer.getMoney() >= price then
            ...
            TriggerClientEvent("pNotify:SendNotification", _source, {text =  'Şu kadar fiyata market satın aldın ' ..  price,type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
        else    
            TriggerClientEvent("pNotify:SendNotification", _source, {text =  "Bu markete paran yetmiyor",type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
        end
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    MySQL.Async.fetchAll(
        ...
        if os.time() - result[1].LastRobbery <= 900 then
            ...
            TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text =  "Dukkan paranız soygun nedeniyle kilitlendi, lütfen bekleyiniz" .. math.floor((900 - time) / 60) .. ' dakika' ,type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
            return
        end
        ...
        if xPlayer.getMoney() >= amount then
            ...
            TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text =  '~g~Marketine ' .. amount .. ' $ koydun' ,type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
        else
            TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text =  "Sahip olduğundan daha fazla koyamazsın" ,type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
        end
    end)
end)
  ...
  MySQL.Async.fetchAll(
    ...
    if os.time() - result[1].LastRobbery <= 900 then
        ...
        if result[1].money >= amount then
            ...
            TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text =  'Marketinden ' .. amount .. '$ aldın' ,type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
            xPlayer.addMoney(amount)
        else
            TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text =  'Sahip olduğundan fazla eşya koyamazsın' ,type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
        end
    ...
    function(result)
      ...
      if result[1].money == 0 and result2[1] == nil then
        MySQL.Async.fetchAll("UPDATE owned_shops SET identifier = @identifiers, ShopName = @ShopName WHERE identifier = @identifier AND ShopNumber = @Number",
        {
            ...
            TriggerClientEvent('esx_kr_shops:removeBlip', -1)
            TriggerClientEvent('esx_kr_shops:setBlip', -1)
            TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text =  "Marketini sattın",type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
            else
            TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text =  "Dukkanın icinde para veya eşya olursa dukkanını satamazsın",type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
            end
        end)
    end)
end)
