...
Citizen.CreateThread(function()
  ...
  while true do
	...
	if (IsControlJustPressed(1, 7)) then
		...
		notowned = 0
		if #cars == 0 then
			exports['chubbsnotify']:DoHudText('inform', ("Yakında Araç Yok."))
		else
			for j=1, #cars, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							exports['chubbsnotify']:DoHudText('error', ('Araç Kilitlendi.'))
						...
						elseif lock == 2 then
							...
							TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10, 'Lock', 0.1)
							exports['chubbsnotify']:DoHudText('inform', ('Aracın Kilidi Açıldı.'))
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							hasAlreadyLocked = true
					...
					if notowned == #carstrie then
						exports['chubbsnotify']:DoHudText('inform', ("Bu araca sahip değilsin."))
					end	
				end, plate)
			end			
		end
