...
TriggerEvent('esx_society:registerSociety', 'cardealer', _U('car_dealer'), 'society_cardealer', 'society_cardealer',
    'society_cardealer', {
        type = 'private'
    })
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function(source, cb, vehicleModel)
    ...
    if xPlayer.job.name == 'cardealer' then
        local paraiadesi = vehicleData.price / 2
        if xPlayer.getMoney() >= paraiadesi then
            TriggerClientEvent('esx:showNotification', source, 'Galerici Olduğun İçin Arabayı %50 İndirimle Aldın.')
            xPlayer.removeMoney(paraiadesi)
            cb(true)
        else
            cb(false)
        end
                    ...
                    if result[1] then -- does the owner match?
                        ...
                        if vehicle.model == model then
                            if vehicle.plate == plate then
                                ...
                                print(('esx_vehicleshop: %s attempted to sell an vehicle with plate mismatch!'):format(
                                          xPlayer.identifier))
                                cb(false)
                            end
                        else
                            print(('esx_vehicleshop: %s attempted to sell an vehicle with model mismatch!'):format(
                                      xPlayer.identifier))
                            cb(false)
                        end
                    else
                                    ...
                                    if result[1] then
                                        ...
                                        if vehicle.model == model then
                                            if vehicle.plate == plate then
                                                ...
                                                print(
                                                    ('esx_vehicleshop: %s attempted to sell an vehicle with plate mismatch!'):format(
                                                        xPlayer.identifier))
                                                cb(false)
                                            end
                                        else
                                            print(
                                                ('esx_vehicleshop: %s attempted to sell an vehicle with model mismatch!'):format(
                                                    xPlayer.identifier))
                                            cb(false)
                                        end
                                    else
...
ESX.RegisterServerCallback("esx_vehicleshop:cardealerReq", function(source, cb, safe)
    ...
    if player then
        ...
        for i = 1, #players do
            ...
            if player and player["job"]["name"] == "cardealer" then
...
ESX.RegisterServerCallback('cardeallerRequired', function(source, cb)
...
RegisterCommand('araçdevret', function(source, args)
    ...
    if (GetPlayerName(tonumber(args[1]))) then
        ...
        TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Gecersiz Oyuncu ID!")
        return
    end
    ...
    MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE plate = @plate', {
        ...
        if result[1] ~= nil then
            ...
            CarOwner = playerName
            print("Arac Devretme ", myID, CarOwner)
            if myID == CarOwner then
                print("Transfer edildi")

                data = {}
                TriggerClientEvent('chatMessage', other, "^4VPlakalı Araç ^*^1" .. plate ..
                    "^r^4tarafindan sana teslim edildi: ^*^2" .. myName)
                ...
                TriggerClientEvent('chatMessage', source,
                    "^4Aracinizi ^*^3plaka^0^4 ile teslim ettiniz ^*^1" .. plate .. "\" ^r^4teslim edilen ^*^2" ..
                        targetSteamName)
            else
                print("Transfer Etmedi")
                TriggerClientEvent('chatMessage', source, "^*^1Araca sahip degilsiniz")
            end
        else
            TriggerClientEvent('chatMessage', source,
                "^1^*ERROR: ^r^0Bu arac plakasi mevcut degil veya yanlisa yazilmis.")
        end
