local Keys = {
  ...
  function OpenShopMenu()
	  ...
	  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
				  ...
				  if Config.EnablePlayerManagement then
					  ESX.TriggerServerCallback('esx_vehicleshop:buyVehicleSociety', function(hasEnoughMoney)
						  if hasEnoughMoney then
							  ...
							  SetEntityCoords(playerPed, -58.439586639404, -1097.4985351563, 26.422369003296)
		  ...
		  FreezeEntityPosition(playerPed, false)
		  SetEntityVisible(playerPed, true)
		  SetEntityCoords(playerPed, -33.7782, -1102.08, 26.422)   
		  --SetEntityCoords(playerPed, Config.Zones.ShopEntering.Pos.x, Config.Zones.ShopEntering.Pos.y, Config.Zones.ShopEntering.Pos.z)
  ...
  -- Create Blips
  Citizen.CreateThread(function()
	  local blip = AddBlipForCoord(-50.69, -1108.39, 60.49)
  ...
  Citizen.CreateThread(function()
	  while true do
		Citizen.Wait(0)
		  if Vdist2(GetEntityCoords(PlayerPedId(), false), -33.5176, -1102.09, 26.422) < 4 then
			  DrawText3Ds(-33.5176, -1102.09, 26.422, "[E] Galerici Salim Abi")
			  if IsControlJustReleased(0, Keys['E']) then
				  --ESX.TriggerServerCallback('cardeallerRequired', function(count)
					  if ESX.PlayerData.job.name == 'cardealer' then
						  OpenShopMenu()
					  end
						 -- OpenShopMenu()
						  --exports["mythic_notify"]:SendAlert("error", "Galerici olmadan bu menüye erişemezsin arabalara ve fiyatlara göz atmak için arkada ki kapıdan showroom'a git")
					  --end
				  --end)
				  ESX.TriggerServerCallback("esx_vehicleshop:cardealerReq", function(state)
					  if state then
						  --OpenShopMenu()
					  else
							if ESX.PlayerData.job.name == 'cardealer' then
								print('Galerici olarak magazaya giris yaptin')
							else
								exports["mythic_notify"]:SendAlert("error", "Galerici olmadan bu menüye erişemezsin arabalara ve fiyatlara göz atmak için arkada ki kapıdan showroom'a git")
							end
						end
  ...
  --     while true do
  --       Citizen.Wait(0)
  --         if Vdist2(GetEntityCoords(PlayerPedId(), false), -57.18, -1098.93, 27.42) < 4 then
  --             DrawText3D(-57.18, -1098.93, 27.35, "[E] Galerici Nigel ")
  --             if IsControlJustReleased(0, Keys['E']) then
  --                 ESX.TriggerServerCallback('cardeallerRequired', function(count)
  --                     if count < Config.cardeallerRequired then
  --                         OpenShopMenu()
  --                     else
  --                         TriggerEvent('notification', 'Zaten Sehirde Galerici Var', 2)
  --                     end
  --                 end)
  --             end
  --         end
