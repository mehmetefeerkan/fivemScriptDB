    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'lsrp-motels',
        {
            ...
            elements = { 
                ...
                { label = 'Kirayi Iptal Et', value = 'cancel' }
            }
        },
    function(data, entry)
...
AddEventHandler('lsrp-motels:roomMenu', function(room, motel)
    ...
    for k,v in pairs(Config.Zones) do
        ...
        table.insert(options, {label = 'Odayi Terket', value = 'leaveroom'})
        if roomOwner == playerIdent then
        table.insert(options, {label = 'Dolabini Ac', value = 'inventory'})
