...
Citizen.CreateThread(function()
    while true do
       -- sleep = 1000 --Citizen.Wait(0)
        local playerPed = PlayerPedId()
        local playerVeh = GetVehiclePedIsIn(playerPed, false)
        --if IsPedInAnyPoliceVehicle(playerPed) then
        --    sleep = 0
        if not isVisible and IsControlJustPressed(0, 178) and GetEntitySpeed(playerVeh) < 5.0 then
...
RegisterNetEvent('police.togglemdt')
AddEventHandler('police.togglemdt', function()
        local playerPed = PlayerPedId()
        local playerVeh = GetVehiclePedIsIn(playerPed, false)
        --if IsPedInAnyPoliceVehicle(playerPed) then
        --    sleep = 0
        if not isVisible and GetEntitySpeed(playerVeh) < 5.0 then
            if GetVehicleNumberPlateText(getVehicleInFront()) then
                TriggerServerEvent("mdt:performVehicleSearchInFront", GetVehicleNumberPlateText(getVehicleInFront()))
            else
                TriggerServerEvent("mdt:hotKeyOpen")
