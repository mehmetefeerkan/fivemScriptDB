...
CCTVCamLocations = {
	...
	[22] =  { ['x'] = -627.46,['y'] = -229.45,['z'] = 40.2,['h'] = 163.15, ['info'] = ' Cam Jewelry Store' },
...
ESX.UI.Menu.Open( 'default', GetCurrentResourceName(), 'cctvm',
  {
    ...
    align = 'top-left',
    elements = { 
      {label = ('Kamera 1'),     value = 'camenu1'},
      {label = ('Kamera 2'),     value = 'camenu2'},
	  {label = ('Kamera 3'),     value = 'camenu3'},
	  {label = ('Kamera 4'),     value = 'camenu4'},
	  {label = ('Kamera 5'),     value = 'camenu5'},
	  {label = ('Kamera 6'),     value = 'camenu6'},
	  {label = ('Kamera 7'),     value = 'camenu7'},
	  {label = ('Kamera 8'),     value = 'camenu8'},
	  {label = ('Kamera 9'),     value = 'camenu9'},
	  {label = ('Kamera 10'),     value = 'camenu10'},
      {label = ('Kamera 11'),     value = 'camenu11'},
      {label = ('Kamera 12'),     value = 'camenu12'},
	  {label = ('Kamera 13'),     value = 'camenu13'},
	  {label = ('Kamera 14'),     value = 'camenu14'},
	  {label = ('Kamera 15'),     value = 'camenu15'},
	  {label = ('Kamera 16'),     value = 'camenu16'},
	  {label = ('Kamera 17'),     value = 'camenu17'},
	  {label = ('Kamera 18'),     value = 'camenu18'},
	  {label = ('Kamera 19'),     value = 'camenu19'},
	  {label = ('Kamera 20'),     value = 'camenu20'},
	  {label = ('Kamera 21'),     value = 'camenu21'},
	  {label = ('Kamera 22'),     value = 'camenu22'},
	  {label = ('Kamera 23'),     value = 'camenu23'},
	  {label = ('Kamera 24'),     value = 'camenu24'}
    }
  },
  function(data, menu)
    if data.current.value == 'camenu1' then
      TriggerEvent('cam1')
	elseif data.current.value == 'camenu2' then
	  TriggerEvent('cam2')
	elseif data.current.value == 'camenu3' then
	  TriggerEvent('cam3')
	elseif data.current.value == 'camenu4' then
	  TriggerEvent('cam4')
	elseif data.current.value == 'camenu5' then
	  TriggerEvent('cam5')
	elseif data.current.value == 'camenu6' then
	  TriggerEvent('cam6')
	elseif data.current.value == 'camenu7' then
	  TriggerEvent('cam7')
	elseif data.current.value == 'camenu8' then
	  TriggerEvent('cam8')
	elseif data.current.value == 'camenu9' then
	  TriggerEvent('cam9')
	elseif data.current.value == 'camenu10' then
	  TriggerEvent('cam10')
	elseif data.current.value == 'camenu11' then
	  TriggerEvent('cam11')
	elseif data.current.value == 'camenu12' then
	  TriggerEvent('cam12')
	elseif data.current.value == 'camenu13' then
	  TriggerEvent('cam13')
	elseif data.current.value == 'camenu14' then
	  TriggerEvent('cam14')
	elseif data.current.value == 'camenu15' then
	  TriggerEvent('cam15')
	elseif data.current.value == 'camenu16' then
	  TriggerEvent('cam16')
	elseif data.current.value == 'camenu17' then
	  TriggerEvent('cam17')
	elseif data.current.value == 'camenu18' then
	  TriggerEvent('cam18')
	elseif data.current.value == 'camenu19' then
	  TriggerEvent('cam19')
...
AddEventHandler("cctv:camera", function(camNumber)
	...
	if inCam then
		...
		if camNumber > 0 and camNumber < #CCTVCamLocations+1 then
			...
			-- exports['mythic_notify']:SendAlert('error', "Güvenlik kamerası bozulmuş görünüyor")
			TriggerEvent('notification', 'Güvenlik kamerası bozulmuş görünüyor', 2)
		end
		end
		else
